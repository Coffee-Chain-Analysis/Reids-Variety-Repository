...
starbuck_rev <- starbuck_rev %>% 
  select(`Americas`, `Europe/Middle-East Asia`, `China/Asia Pacific`, `Total-Licensed`, year) %>% 
  gather(Type, count, Americas, `Europe/Middle-East Asia` ,`China/Asia Pacific` , `Total-Licensed`) %>% 
  na.omit()
...
# Starbuck's Total Revenue Analysis
```{r revenue_starbucks_19, message = FALSE, warning = FALSE}
starbuck_rev<- read_csv("data/starbucks.csv")
...
starbuck_rev <- starbuck_rev %>% 
  rename(`Company Operated Stores` = Company_Operated_Stores, `Licensed Stores` = License_Stores)

starbuck_rev <- starbuck_rev %>% 
  select(`Company Operated Stores`, `Licensed Stores`, Other, Total,year) %>% 
  gather(Type, count, `Company Operated Stores`, `Licensed Stores`, Other, Total) %>% 
  na.omit()
